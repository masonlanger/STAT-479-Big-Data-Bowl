...
To get points saved above replacement, we have to consider EPA which is estimated points allowed. The previous model used a binary outcome of incompletion or not to measure success, but to get points saved, we shift to using Points modeled by a Gaussian instead. The approach to get points saved above replacement will be to calculate the EPA from predictors from the GAM regression model and then find EPA if every pass was completed, then subtracting expected points added from this to get overall points saved.
```{r}
df_plays_subset <- df_plays %>%
  select(game_id, play_id, expected_points, expected_points_added, pass_result)
df_final <- df_total %>%
  select(-pass_result) %>% 
  left_join(df_plays_subset, by = c("game_id", "play_id"))


df_final <- df_final %>%
  mutate(
    # Outcome category from pass_result
    outcome_category = case_when(
      pass_result == "C"  ~ "Completed",
      pass_result == "I"  ~ "Incomplete",
      pass_result == "IN" ~ "Interception",
    ...
    outcome_category = factor(outcome_category,
                              levels = c("Completed","Incomplete","Interception","Other")),
    
    # Categorical encodings as factors (they look numeric in your snippet)
    pass_location_type      = factor(pass_location_type),
    route_of_targeted_receiver = factor(route_of_targeted_receiver),
    defenders_in_the_box    = factor(defenders_in_the_box),
    team_coverage_type      = factor(team_coverage_type),
    dropback_type           = factor(dropback_type)
  )
...
df_completed <- df_model %>%
  filter(outcome_category == "Completed")
```

Now we fit the model to find what the EPA would be if the pass was completed

```{r}
gam_epa <- gam(
  expected_points_added ~ ats_score + min_separation_achieved + final_dist_to_ball + dist_traveled + initial_dist_to_ball + dist_closed + pem_score + smv_score + pass_location_type + route_of_targeted_receiver + defenders_in_the_box + team_coverage_type + dropback_type,
  data   = df_completed,
  family = gaussian()
)
...
```
We can see the significant predictors from this model as well, like ats, dist_traveled and pem, which connects our previous metrics to calculating this one. 

The points saved is calculated by taking the epa if completed - the expected points added, effectively leaving us with how many points were saved if the pass was not complete(interception or incomplete)

```{r}
df_model <- df_model %>%
  mutate(
    # Counterfactual EPA if this pass were completed
    epa_if_completed = predict(gam_epa, newdata = df_model),
    # Defensive points saved on the actual outcome
    def_points_saved = epa_if_completed - expected_points_added
  )

# Quick sanity check by outcome
df_model %>%
  group_by(outcome_category) %>%
  summarise(
    mean_epa_if_completed = mean(epa_if_completed),
    mean_actual_epa      = mean(expected_points_added),
    mean_points_saved    = mean(def_points_saved),
    .groups = "drop"
  )

```

From this we see that if pass was completed, the points saved is very close to 0. If the pass was incomplete, the points saved is a lot higher, at about 2.5. If the pass was intercepted, the points saved is very high at 6.5 which means that per interception, they are saving about a touchdown worth of points. This shows how interceptions have a huge influence in the points saved model, which can play a big part in shaping the results. 

Now using the Points saved, we fit another regression model to fit the replacement level of a player for points saved. Using this replacement level, we will calculate players above and below this level to find the most effective players at saving points and the least effective


```{r}

gam_ps <- gam(
  def_points_saved ~ ats_score + min_separation_achieved + final_dist_to_ball + dist_traveled + initial_dist_to_ball + dist_closed + pem_score + smv_score + pass_location_type + route_of_targeted_receiver + defenders_in_the_box + team_coverage_type + dropback_type,
  data   = df_model,
  family = gaussian()
)
...
df_model <- df_model %>%
  mutate(
    baseline_ps = predict(gam_ps, newdata = df_model)
  )

df_model <- df_model %>%
  mutate(
    closest_defender_id = as.numeric(closest_defender_id)
  )

#mixed model uses points saved and learns baseline
ml_points <- lmer(
  def_points_saved ~ baseline_ps +
    (1 | closest_defender_id) +
    (1 | team_coverage_type) +
    (1 | route_of_targeted_receiver),
  data = df_model
)
...
```{r}
def_u <- ranef(ml_points)$closest_defender_id %>%
  tibble::rownames_to_column("closest_defender_id") %>%
  mutate(
    closest_defender_id = as.numeric(closest_defender_id)
  ) %>%
  rename(u = `(Intercept)`)

# Number of plays per defender
def_counts <- df_model %>%
  group_by(closest_defender_id) %>%
  summarise(
    n_plays = n(),
...
def_u_counts <- def_u %>%
  left_join(def_counts, by = "closest_defender_id")

#defenders with fewer than 45 plays filtered
repl_pool <- def_u_counts %>%
  filter(n_plays < 45)

# Replacement-level effect: e.g., 20th percentile of that pool
repl_u <- quantile(repl_pool$u, probs = 0.2, na.rm = TRUE)

print(paste("Replacement-level random effect (u):", round(repl_u, 4)))
...
df_value <- df_model %>%
  left_join(def_u, by = "closest_defender_id") %>%
  mutate(
    # Defenders with no estimated effect get 0 (full shrinkage)
    u = ifelse(is.na(u), 0, u),
    repl_u = as.numeric(repl_u),
    
    # Expected points saved for this defender vs replacement
    fitted_ps = baseline_ps + u,
    repl_ps   = baseline_ps + repl_u,
    value_ps  = fitted_ps - repl_ps   # points saved above replacement on this play
  )
...
player_value_points <- df_value %>%
  group_by(closest_defender_id) %>%
  summarise(
    n_plays           = n(),
    total_points_saved_above_repl = sum(value_ps),
    avg_points_saved_above_repl   = mean(value_ps),
    .groups = "drop"
  ) %>%
  left_join(df_defender_info,
            by = c("closest_defender_id" = "nfl_id")) %>%
  # Require a minimum number of plays for stability
  filter(n_plays >= 45) %>%
  arrange(desc(total_points_saved_above_repl))

cat("Top 20 defenders by total points saved above replacement:\n")
top_points = player_value_points %>%
  head(20) %>%
  select(player_name, player_position, n_plays,
         total_points_saved_above_repl,
         avg_points_saved_above_repl) %>%
  print()
...
```{r}
cat("\nTop 20 defenders by *average* points saved above replacement (min 45 plays):\n")
top_avg = player_value_points %>%
  arrange(desc(avg_points_saved_above_repl)) %>%
  head(20) %>%
  select(player_name, player_position, n_plays,
         avg_points_saved_above_repl) %>%
  print()
...
worst20_defenders_points <- player_value_points %>%
  arrange(avg_points_saved_above_repl) %>%   # ascending order = worst
  slice_head(n = 20)

```

From these results we see the top 20 best average points saved, and total points saved per defender. We also see the bottom 20 average points saved per defender. From these results we see that ball hawking corners are placed the highest, for example DaRon bland is known for interceptions. There are also very good corners on this list like Denzel Ward and Deommodore Lenoir. From this results, we don't necessarily see the best corners all the time, but they are all generally pretty good. There is a linebacker, Foye Oluokun who is a really good coverage linebacker, with good pursuit speed. Since the PEM is an explanatory variable, it plays a big part in the results, showing very high pursuit corners, although their coverage may not be that tight. The worst defenders are consistent with some of the worst corners in the NFL and lineabckers who are getting beat a lot like Carlton Davis and Nate Hobbs. There are some mismatches like consensus top corner Pat Surtain is in the bottom list, possibly because he is always very close to his receiver and doesn't have high pursuit.  
